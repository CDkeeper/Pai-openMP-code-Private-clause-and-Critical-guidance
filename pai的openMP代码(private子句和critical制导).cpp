...
int main() {
    ...
    double tmpcal; //保存当前计算结果
    omp_set_num_threads(NUM_THREADS); //设置线程数目
    startTime = clock(); //程序开始
    #pragma omp parallel private(i,tmpcal) // 编译制导语句，由制导标识符，制导名称，子句以及括号内传入参数等
    {
        int tidnub = omp_get_thread_num(); //获取当前线程号，如果在外部定义tidnub就得以私有变量传入，因为线程号不共享
        tmpcal = 0.0; //切记tmpcal是作为私有变量传入并行域内的，否则每一个线程都能更改其值
        for (i = tidnub; i < N; i += NUM_THREADS) { //如果i从0开始那么就到N-1结束否则结果不准确
